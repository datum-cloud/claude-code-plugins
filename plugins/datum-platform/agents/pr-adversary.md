...
- **Check every name the body uses.** Each site, cluster, namespace, resource, and alert the body or its examples name has to exist in live state or in the render at the head. An author working from memory writes names that look right and are not, and a reader cannot tell. A name that resolves to nothing is a blocker, since the example then describes a system nobody runs.
